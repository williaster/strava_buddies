...
N_ACTIVITIES = 100 # number of activities to fetch for the athlete, some will
                  # be filtered based on activity type, manual upload, etc.
...
def get_user_activities(client, **kwargs):
    ...
    """
    filtered_summaries = []
    kwargs["id"]       = True # only return ids
...
def add_activity(conn, athlete_id, Activity):
    ...
    datetime  = Activity.start_date.strftime('%Y-%m-%d %H:%M:%S')
    statement = "INSERT INTO %s VALUES (%i, %i, '%s', %.2f, %.2f, %s);" % \
                (table, athlete_id, Activity.id, Activity.type, distance, \
                 elevation, statement)
    try:
        cur = conn.cursor()
        ...
        logger.critical("Error with activity %i for %i" % (Activity.id, athlete_id))
...
def main():
    ...
    for i in len(args.access_tokens): # for each athlete
        client              = Client()
        ...
        activity_ids = get_user_activities(conn, athlete_id) 
        ...
        for act_id in act_summaries:
            ...
            add_activity(conn, athlete_id, detailed_activity)
            ...
            for seg_effort in detailed_activity.segment_efforts:
                ...
                activity_to_segments[act_id][seg_id].append( seg_dct )
